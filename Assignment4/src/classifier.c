...
#include <stdlib.h>
#include <math.h>
int classifier(parameter param,double** input,double** output,int train_record)
 {
 ...
 for(int i=0;i<param.n_features;i++)
  ...
  for(int i=1;i<param.n_layers+1;i++)
   ...
   sigmoid(input[param.n_layers+1],output[param.n_layers+1],param.n_nodes_layer[param.n_layers-1]);
if(fabs(output[param.n_layers+1][0]- correct_value)>0.5)
 return 0;
 else
 return 1;  
    }
